...
def sharpen_3x3(src, out, W: ct.Constant[int]):
    ...
    center = ct.gather(src, (c, h, offsets)).astype(ct.int16)
    up = ct.gather(src, (c, h - 1, offsets)).astype(ct.int16)
    down = ct.gather(src,(c, h + 1, offsets)).astype(ct.int16)
    left = ct.gather(src, (c, h, offsets-1)).astype(ct.int16)
    right = ct.gather(src, (c, h, offsets+1)).astype(ct.int16)
    ...
    ct.scatter(out, (c, h, offsets), val)
